...
imap <- IntervalMap(c(10, 15, 30),
                    ends = c(20, 25, 40),
                    values = c("Gene A", "Gene B", "Gene C"))
```

Alternatively data can be added sequentially, followed by calling `build()`:
...
# Find all indices that overlap with range 12-18
indices <- search_idxs(imap, 12, 18)
print(indices)
```
